...
select productid, sum(saleamount) as total, avg(saleamount) as avgsales from sales 
group by productid
...
--13.Write a query that finds the highest and lowest Salary by 
--department in the Employees table.(DeptID is enough, if you don't have DeptName).
...
--15.Write a query to show the AVG salary and COUNT(*) of employees
--working in each department.(DeptID is enough, if you don't have DeptName).
select departmentname, avg(salary) as avg_salary, count (employeeid) as emp_number from employees
group by departmentname
...
select saledate, sum(saleamount) as total from sales 
group by saledate
...
select customerid , count(quantity) as order_number from orders group by customerid 
having count(quantity)>=3

--19.Write a query to filter out Departments with average salary expenses 
--greater than 60000.(DeptID is enough, if you don't have DeptName).
select * from employees
...
--Hard-Level Tasks (6)
--20.Write a query that shows the average price for each product 
--category, and then filter categories with an average price greater than 150.
...
group by customerid 
having sum(saleamount)>150

--22.Write a query to find the total and average salary of employees in each department, and filter the 
--output to include only departments with an average salary greater than 65000.
select * from employees
select departmentname, sum(salary), avg(salary) from employees 
group by departmentname having avg(salary)>65000

--23.Write a query to find total amount for the orders which weights more than $50 for each customer along 
--with their least purchases.(least amount might be lower than 50, 
--use tsql2012.sales.orders table,freight col, ask ur assistant to give the TSQL2012 database).


--24 Write a query that calculates the total sales and counts unique products sold in each 
--month of each year, and then filter the months with at least 2 products sold.(Orders)
select * from orders
select year(orderdate) as year,
month(orderdate) as month,
sum(totalamount) as total_sales,
count(distinct productId) as unique_product_sold
...
select year(orderdate) as year,
min(quantity) as min,
max(quantity) as max from orders 
