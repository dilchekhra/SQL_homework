...
select category , sum(stockquantity) as total from products group by category
--2.Using Products table, get the average price of products in the 'Electronics' category.
select category, avg(price) as avg_price from products group by category having category='electronics'
...
else 'sufficient' end as Status from products
...
select * from orders
select avg(quantity) as avg_or_amount, year(orderdate) as year from orders group by year(orderdate)
...
when price between 100 and 500 then 'mid' end as status
from products
...
select * from sales
select productId as productid, sum(saleamount) as total from sales group by productid 
...
select * from invoices
select top 3 customerid as customerid, max(totalamount) as totalspent from invoices group by  customerid
